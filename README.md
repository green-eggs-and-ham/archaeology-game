...
- The temporary trench diagnostics include **Depths** plus a **Performance: Full/Lite** switch. Full keeps curved terrain and clipped AO, rendering it in a background worker where supported while an immediate Lite preview keeps input responsive; Lite uses cached rectangular cells and inexpensive cardinal-edge shading for lower-end hardware.
...
- The water-card comparison control switches between a fully visible immersed find and the default realistic view, where the bucket front hides the submerged portion and a dashed outline keeps its position legible.
